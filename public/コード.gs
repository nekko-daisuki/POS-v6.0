...
function saveOrder(data) {
  const ss = SpreadsheetApp.openById('1V2YyBjvv2P3N-quLNDZIaDwlKPIKIUXNTjuv6dvAzKc');
  const salesSheet = ss.getSheetByName('売上データ') || ss.insertSheet('売上データ');
  
  // ヘッダー行の確認と設定
  if (salesSheet.getLastRow() === 0) {
    salesSheet.appendRow(['ID', '日時', 'テーブル番号', '商品名', '単価', '数量', '小計', 'ステータス']);
  ...
  const orderIdBase = timestamp.getTime(); // 注文ごとのベースID

  // 注文アイテムごとにデータを追加
  data.items.forEach(function(item, index) {
    const uniqueId = `${orderIdBase}_${index}`; // 各項目にユニークIDを付与
  ...
  // 会計サマリー情報も記録
  const summarySheet = ss.getSheetByName('会計サマリー') || ss.insertSheet('会計サマリー');
  if (summarySheet.getLastRow() === 0) {
    summarySheet.appendRow(['日時', 'テーブル番号', '合計点数', '合計金額', '預かり金額', 'お釣り', '注文内容']);
    summarySheet.setColumnWidth(1, 180);
    summarySheet.setColumnWidth(2, 100);
    summarySheet.setColumnWidth(7, 300);
  }
  
  const orderDetails = data.items.map(item => `${item.name} x${item.quantity}`).join(', ');
  ...
  return ContentService.createTextOutput(JSON.stringify({ success: true, message: '注文データを保存しました' }))
    .setMimeType(ContentService.MimeType.JSON);
}
...
function updateOrderStatus(data) {
  ...
  for (let i = 1; i < values.length; i++) {
    if (values[i][0] == data.uniqueId) { // IDが一致
      sheet.getRange(i + 1, 8).setValue(data.newStatus); // ステータス列（8番目）を更新
      return ContentService.createTextOutput(JSON.stringify({ success: true, message: 'ステータスを更新しました' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
  }
  
  return ContentService.createTextOutput(JSON.stringify({ success: false, message: '対象の注文が見つかりません' }))
    .setMimeType(ContentService.MimeType.JSON);
}
  ...
  try {
    switch (e.parameter.action) {
      ...
      case 'getOrders':
        return getOrders();
      default:
        return ContentService.createTextOutput(JSON.stringify({ status: 'active', message: 'API is active' }))
          .setMimeType(ContentService.MimeType.JSON);
    }
  } catch (error) {
    return ContentService.createTextOutput(JSON.stringify({ success: false, error: error.toString() }))
